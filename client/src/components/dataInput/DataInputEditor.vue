...
type DataInputEditorProps = {
    dataInput?: DataInput;
};

const props = defineProps<DataInputEditorProps>();

const emit = defineEmits([ 'save', 'cancel', 'delete' ]);

const fetching = ref(false);
const innerValue = ref<DataInputUpdate>(props.dataInput ? copyDataInputUpdate(props.dataInput) : getNewDataInputUpdate());
...
async function createNew() {
    ...
    const result = await API.dataInputs.createDataInput({}, init);
    if (result.status)
...
async function updateOld() {
    ...
    const result = await API.dataInputs.updateDataInput({ id: props.dataInput.id }, innerValue.value);
    if (result.status)
        emit('save', result.data);
}
...
async function deleteMethod() {
    ...
    fetching.value = true;
    const result = await API.dataInputs.deleteDataInput({ id: props.dataInput.id });
    if (result.status)
        emit('delete');
