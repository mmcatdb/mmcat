...
public class ResultStructureMerger {
     ...
     */
    public static MergeTform run(ResultStructure sourceRoot, ResultStructure targetRoot, ResultStructure targetProperty, ResultStructure sourceMatch, ResultStructure targetMatch) {
        return new ResultStructureMerger(sourceRoot, targetRoot).run(targetProperty, sourceMatch, targetMatch);
    }

    private final ResultStructure sourceRoot;
    private final ResultStructure targetRoot;

    private ResultStructureMerger(ResultStructure sourceRoot, ResultStructure targetRoot) {
    ...
    private MergeTform run(ResultStructure targetProperty, ResultStructure sourceMatch, ResultStructure targetMatch) {
        ...
        current.addChild(new WriteToIndex<MapResult>(index, sourceToMatchKeys));
    ...
    private ResultStructure createNewStructure(ResultStructure targetProperty, List<ResultStructure> targetToMatch, List<ResultStructure> sourceToMatch, List<String> deleteKeys) {
        ...
        ResultStructure current = targetPropertyCopy;
        for (int i = 0; i < deleteKeys.size() - 1; i++)
