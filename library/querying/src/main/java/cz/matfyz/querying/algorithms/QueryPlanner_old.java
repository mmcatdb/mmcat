...
import cz.matfyz.querying.core.QueryPart_old;
 ...
 */
@Deprecated
public class QueryPlanner_old {

    private final SchemaCategory schema;
    private final List<Kind> allKinds;

    public QueryPlanner_old(SchemaCategory schema, List<Kind> allKinds) {
        this.schema = schema;
    ...
    private QueryPlan createPlanFromAssignment(Query query, Map<String, SchemaObject> variableTypes, List<TripleKind> assignment) {
        var initialQueryPart = new QueryPart_old(assignment.stream().toList(), new ArrayList<>());

        var finishedQueryParts = new ArrayList<QueryPart_old>();
        var queryPartQueue = new LinkedList<QueryPart_old>();
     ...
     */
    private List<QueryPart_old> splitSingleQueryPart(Map<String, SchemaObject> variableTypes, QueryPart_old queryPart) {
    ...
    static record QueryPartKind(
        QueryPart_old queryPart,
        Kind kind
    ) {}

    private List<QueryPart_old> splitJoinPoint(
        ...
        var queryPartA = new QueryPart_old(triplesMappingA, new ArrayList<>());
        var queryPartB = new QueryPart_old(triplesMappingB, new ArrayList<>());
