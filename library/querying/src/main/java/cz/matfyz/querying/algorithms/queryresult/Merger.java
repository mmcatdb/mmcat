...
public class Merger {
    ...
    private QueryResult run(JoinCandidate join) {
        ...
        final ResultStructure fromStructure = findStructure(fromResult, condition.from());
        final ResultStructure toStructure = findStructure(toResult, condition.to());

        throw new UnsupportedOperationException();
    }

    private ResultStructure findStructure(QueryResult result, Signature signature) {
        ...
        final ResultStructure output = GraphUtils.findDFS(result.structure, s -> s.isLeaf() && s.schemaObject.equals(schemaObject));
        if (output == null)
