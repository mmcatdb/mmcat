...
public class MongoDBQueryWrapper extends BaseQueryWrapper implements AbstractQueryWrapper {
    ...
    private String baseCollectionName() {
        ...
        if (kindNames.size() > 1)
            throw QueryException.message("Joining collections in MongoDB is not supported since mmcat does not support it!");
    ...
    private Map<String, Map<String, Integer>> buildProjection() {
        ...
        // We support nested complex properties even for arrays, but mmcat does not support them yet, so their querying will not work.
