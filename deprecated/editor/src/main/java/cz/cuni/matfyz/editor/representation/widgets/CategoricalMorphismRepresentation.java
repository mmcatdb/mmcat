...
import cz.cuni.matfyz.editor.representation.EdgeRepresentation;
 ...
 * @author pavel.koupil
 */
public class CategoricalMorphismRepresentation extends EdgeRepresentation {
	...
	public CategoricalMorphismRepresentation(CategoricalMorphismWidget model, WidgetRepresentation source, WidgetRepresentation target) {
		super(model, source, target);

		source.addCellChild(target);
		target.addCellParent(source);		// POTREBUJES VICE PARENTS!

		line = new Line();
		line.setStrokeWidth(model.getStrokeWidth());
		...
		Text edgeId = new Text(model.getId());
