...
The [previous section]({{% relref "/aimm/motivation-and-concepts" %}}) introduced the main idea behind database mappings: the same conceptual data can be represented in several valid ways across relational, document, and graph databases. This section describes how the project turns those mapping choices into an optimization problem.
...
A candidate mapping describes how the kinds from the reduced schema are represented in the available database instances.

A kind is a unit of storage relavant to the optimizer, such as a table, collection, node, or. For each kind, a candidate mapping may decide that the kind is:
